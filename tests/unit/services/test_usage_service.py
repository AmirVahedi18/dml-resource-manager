...
from dml_core.services import usage_service as use
...
def test_total_ram_hours_by_user(db_session):
    server = make_server(db_session)
    gpu = make_gpu(db_session, server, total_ram_mb=20000)
    regulation = make_regulation(db_session)
    user = make_user(db_session)
    ...
    reservations = use.get_reservations_in_range(db_session, NOW, NOW + timedelta(days=1))
    totals = use.total_ram_hours_by_user(reservations, NOW, NOW + timedelta(days=1))
    assert totals[user.id] == 2.0 * 4096
