...
SQLAlchemy + bcrypt), following the same pattern as the other services/*.py modules -- the bot never
imports this, since it authenticates by Telegram ID instead (see bot/auth.py), but nothing here is
coupled to either interface."""
import bcrypt
...
def ensure_admin_seeded(session: Session, username: str, password: str) -> User | None:
    ...
    (mirrors ADMIN_IDS' role for the bot -- see auth_service module docstring). Does nothing if an
    account with that username already exists (won't reset its password on every restart); returns
    None in that case so the caller can decide whether to log anything."""
